...
Token Parser::parse_body(Node *parent, Array<StmtNode *> &out, int depth) {
    while (token().type() != tok_desindent && token().type() != tok_eof) {
        ...
        // newlines can hiden a desindent
        // since newlines do not affect indentation
...
#include "parser.h"
        ...
        StmtNode *not_implemented_stmt(Node *parent) {
            return parent->new_object<NotImplementedStmt>();
        }

        ExprNode *not_implemented_expr(Node *parent) {
        ...
        void Parser::parse_comprehension(Node *parent, Array<Comprehension> &out, char kind,
                                         int depth) {
            TRACE_START();
        ...
        ExprNode *parse_comprehension(Parser *parser, Node *parent, ExprNode *child, char kind,
                                      int depth) {
        ...
        ExprNode *parse_dictcomprehension(Parser *parser, Node *parent, ExprNode *key,
                                          ExprNode *value, char kind, int depth) {
        ...
        ExprNode *parse_literal(Parser *parser, Node *parent, ExprNode *child, char kind,
                                int depth) {
            // This is a tuple
        ...
        ExprNode *parse_dictliteral(Parser *parser, Node *parent, ExprNode *key, ExprNode *value,
                                    char kind, int depth) {
            // This is a tuple
        ...
        ExprNode *parse_comprehension_or_literal(Parser *parser, Node *parent, int tok, char kind,
                                                 int depth) {
            // Save the start token to set the code loc when we know if this is a tuple or a
            // generator
        ...
        ExprNode *Parser::parse_set_dict(Node *parent, int depth) {
            return parse_comprehension_or_literal<SetComp, SetExpr>(this, parent, tok_curly, '}',
                                                                    depth);
        }
        ...
        ExprNode *Parser::parse_subscript(Node *parent, ExprNode *primary, int depth) {
            ...
            if (elts.size() == 0) {
                errors.push_back(
                    ParsingError::syntax_error("Substript needs at least one argument"));
            }
        ...
        ExprNode *Parser::parse_slice(Node *parent, ExprNode *primary, int depth) {
            ...
            if (!allow_slice()) {
                errors.push_back(
                    ParsingError::syntax_error("Slice is not allowed in this context"));
