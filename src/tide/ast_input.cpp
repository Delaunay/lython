...
namespace lython {
    ...
    void ASTEditor::input(float dt) {
        ...
        {
            ImGuiKeyData* key_data = ImGui::GetKeyData(ImGuiKey_RightArrow);
            if(!key_data->Down && key_data->DownDurationPrev >= 0)  {
                index = clamp(index + 1, 0, n - 1);
            }
        }

        {
            ImGuiKeyData* key_data = ImGui::GetKeyData(ImGuiKey_LeftArrow);
            if (!key_data->Down && key_data->DownDurationPrev >= 0)  {
                index = clamp(index - 1, 0, n - 1);
            }
        }

        ImGuiKeyData* key_data = ImGui::GetKeyData(ImGuiKey_Backspace);

        
        if (key_data != nullptr && !input_buffer.empty()) {
            // Down & release
            if (!key_data->Down && key_data->DownDurationPrev >= 0) {
    ...
    void ASTEditor::test() {
        ...
        //         drawlist->AddRect(draw.rectangle.Min, draw.rectangle.Max, ImColor(255, 255, 255), 0, 0, 1);
        ...
        for(Group& group: renderer.groups) {
            ...
            // drawlist->AddRect(group.rectangle.Min, group.rectangle.Max, ImColor(255, 255, 255), 0, 0, 1);
        }
    ...
    void ASTEditor::draw(float dt) {
        ...
        for(int i = 0; i < 30; i++) {
            ...
            // drawlist->AddRect(ImVec2(0, py_s), ImVec2(size.x, py_e), ImColor(255, 255, 255), 0, 0, 1);
            drawlist->AddText(ImVec2(size.x - s.size() * size.x / 4, py_s), ImColor(255, 255, 255), s.c_str());
        }
        ...
        if (!suggestions.empty()) {
            ...
            for(int i = 0; i < suggestions.size(); i++) {
                ImVec2 size = style.font->CalcTextSizeA(style.font_size, FLT_MAX, 0.0f, suggestions[i].c_str());
        ...
        // Debug draw
        {
            ...
            for(int k : renderer.edit_order) {
                ...
                ImGui::Text("Group %3d (%3d:%2d) -> (%3d:%2d)", 
                    group->id, 
                    get_line(group->rectangle.Min.y), get_col(group->rectangle.Min.x),
                    get_line(group->rectangle.Max.y), get_col(group->rectangle.Max.x)
                ); // str(group->node).c_str());
        ...
        if (renderer.edit_order.size() > 0) {
            ...
            if (idx >= 0) {
                ...
                drawlist->AddRect(
                    selected->rectangle.Min, 
                    selected->rectangle.Max, 
                    ImColor(255, 0, 0), 
                    0, 
                    0, 
                    1
