...
{ 
    GraphNodeBase* graph = new_object<GraphNode>(n);
    new_input(graph, n->lhs, depth);
    new_input(graph, n->rhs, depth);
    return new_output(graph, depth); 
}
...
ExprRet ToGraph::call(Call_t* n, int depth) {     
    ...
    if (current_exec_pin != nullptr) {
        GraphNodePinBase* exec_prev = new_exec_input(graph);
        GraphNodePinBase* exec_next = new_exec_output(graph);
...
StmtRet ToGraph::functiondef(FunctionDef_t* n, int depth) 
{
    ...
    GraphNodePinBase* exec_next = new_exec_output(args);
...
StmtRet ToGraph::forstmt(For_t* n, int depth) { 
    GraphNodeBase* graph = new_object<GraphNode>(n);
    GraphNodePinBase* exec_prev = new_exec_input(graph);
    GraphNodePinBase* exec_body = new_exec_output(graph);
...
StmtRet ToGraph::whilestmt(While_t* n, int depth) { 
    ...
    GraphNodePinBase* exec_prev = new_exec_input(graph);
    GraphNodePinBase* exec_body = new_exec_output(graph);
    GraphNodePinBase* exec_cond = new_exec_output(graph);
...
StmtRet ToGraph::ifstmt(If_t* n, int depth) { 
    GraphNodeBase* graph = new_object<GraphNode>(n);
    auto exec_prev = new_exec_input(graph);
    new_input(graph, n->test, depth);

    GraphNodePinBase* exec_true = new_exec_output(graph);
    for(Stmt* stmt: body) {
    ...
    GraphNodePinBase* exec_false = new_exec_output(graph);
...
StmtRet ToGraph::trystmt(Try_t* n, int depth) { 
    ...
    GraphNodePinBase* exec_normal = new_exec_ouput(graph);
    for(Stmt* stmt: n->body) {
        current_exec_pin = exec_body;
        current_exec_pin = exec(stmt, depth);
    }
    ...
    for(auto& handler: n->handlers) {
        ...
        GraphNodePinBase* exec_handler = new_exec_ouput(graph);
        for(Stmt* stmt: handler->body) {
    ...
    GraphNodePinBase* exec_orelse = new_exec_ouput(graph);
    for(Stmt* stmt: n->exec_orelse) {
        current_exec_pin = exec_orelse;
        current_exec_pin = exec(stmt, depth);
    }
    join->pins().push_back(current_exec_pin);

    GraphNodePinBase* exec_finalbody = new_exec_ouput(graph);
    for(Stmt* stmt: n->finalbody) {
...
StmtRet ToGraph::match(Match_t* n, int depth) {
    ...
    for(MatchCase& match: n->cases) {
        GraphNodePinBase* output = new_exec_output(graph);
        
        // the match pattern is an input
        // TODO

        // branching
        for(Stmt* stmt: match.body) {
...
StmtRet ToGraph::inlinestmt(Inline_t* n, int depth) { 
    for(Stmt_t* stmt: n->body) {
        current_exec_pin = exec(stmt);
    }
 }
